...
public class Labirinto {
    ...
    private Stack<int[]> caminhoPercorrido;

    public Labirinto(){
        matriz = new ArrayList<>();
        caminhoSolucao = new Stack<>();
        caminhoPercorrido = new Stack<>();
        carregarLabirinto();
    ...
    public void encontrarSolucao(){
        ...
        caminhoPercorrido.push(entrada);
        ...
        while (lAtual != saida[0] && cAtual != saida[1]){
            ...
            while(lBusca <= (lAtual+1) && !achou){
                while(cBusca <= (cAtual+1) && !achou){
                    if ((lBusca >= 0 && lBusca < matriz.size()) && (cBusca >= 0 && cBusca < matriz.get(0).length)){
                        if((matriz.get(lBusca)[cBusca].equals("1")) && !caminhoPercorrido.contains(new int[]{lBusca, cBusca})){
                            caminhoSolucao.push(new int[]{lBusca, cBusca});
                            caminhoPercorrido.push(new int[]{lBusca, cBusca});
                            lAtual = lBusca;
                            cAtual = cBusca;
            ...
            //if(!achou && lAtual != saida[0] && cAtual != saida[1])
    ...
    private void carregarLabirinto() {
        String arquivo = "C:\\Users\\marlo\\IdeaProjects\\TPALabirinto\\maze.csv";
